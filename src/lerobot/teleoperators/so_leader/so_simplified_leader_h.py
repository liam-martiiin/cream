...
from .config_so_leader import SOSimplifiedLeaderHConfig

logger = logging.getLogger(__name__)


class SOSimplifiedLeaderH(Teleoperator):
    """Generic SO leader base for SO-100/101/10X teleoperators."""

    config_class = SOSimplifiedLeaderHConfig
    name = "so_simplified_leader_h"

    def __init__(self, config: SOSimplifiedLeaderHConfig):
        super().__init__(config)
        self.config = config
        self.config.port = resolve_or_verify_port(
            self.id, self.config.port, register_command_hint="--type so_simplified_leader_h"
        )
    ...
    def calibrate(self) -> None:
        ...
        # Fix 3: Remove all 'wrist_roll' logic since your 4-DOF arm doesn't have it!
    ...
    def _ticks_to_radians(self, motor_name: str, centered_ticks: int) -> float:
        import math
        # The bus already applied the offset, so these ticks are centered at 0!
        rads = (centered_ticks / 4096.0) * (2.0 * math.pi)
        # Wrap to strictly -pi to pi (Safely ignores multi-turn hardware windups)
...
SOSimplifiedLeaderH = SOSimplifiedLeaderH

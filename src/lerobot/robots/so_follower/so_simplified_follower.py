...
from .config_so_follower import SO101SimplifiedFollowerConfig

logger = logging.getLogger(__name__)


class SOSimplifiedFollower(Robot):
    """
    ...
    config_class = SO101SimplifiedFollowerConfig
    name = "so_simplified_follower"

    def __init__(self, config: SO101SimplifiedFollowerConfig):
        super().__init__(config)
        self.config = config
        self.config.port = resolve_or_verify_port(
            self.id, self.config.port, register_command_hint="--type so101_simplified_follower"
        )
...
SO101SimplifiedFollower = SOSimplifiedFollower
